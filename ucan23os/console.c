...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		if (finfo->size >= 8 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) { /* 修改文件的权限 */
			segsiz = *((int *) (p + 0x0000));
			esp    = *((int *) (p + 0x000c));
			datsiz = *((int *) (p + 0x0010));
			dathrb = *((int *) (p + 0x0014));
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		sheet_updown(sht, shtctl->top); /* 将窗口图层高度指定为鼠标所在图层的高度，鼠标移到上层 */
		reg[7] = (int) sht;
	}  else if (edx == 6) {
